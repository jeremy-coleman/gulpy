...
streamtest.versions.forEach(version => {
  test.cb(
    ...
    t => {
      ...
      const stream = new FirstChunkStream({ chunkSize: 7 }, async chunk => {
        t.pass()
        return chunk
      })
  ...
  test.cb(`for ${version} streams, throwing errors from callback emits error`, t => {
    t.plan(2)
  ...
  test.cb(`for ${version} streams, requires a 0 length first chunk`, t => {
    ...
    streamtest[version]
      .fromChunks([content])
      .pipe(
        new FirstChunkStream({ chunkSize: 0 }, async chunk => {
  ...
  test.cb(
    ...
    t => {
      ...
      streamtest[version]
        .fromChunks([content])
        .pipe(
          new FirstChunkStream({ chunkSize: 7 }, async chunk => {
  ...
  test.cb(`for ${version} streams, leaves content as is with required size chunk`, t => {
    t.plan(2)
  ...
  test.cb(`for ${version} streams, leaves content as is with several small chunks`, t => {
    t.plan(2)
  ...
  test.cb(`for ${version} streams, works with string and encoding`, t => {
    t.plan(2)
  ...
  test.cb(`for ${version} streams, works with stop`, t => {
    t.plan(2)
