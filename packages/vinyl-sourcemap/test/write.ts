...
describe("write", function () {
  it("errors if file argument is undefined", function (done) {
    sourcemaps.write(undefined, function (err) {
      expect(
        err instanceof Error && err.message === "vinyl-sourcemap-write: Not a vinyl file"
      ).toExist()
      done()
    })
  })
  ...
  it("writes an inline sourcemap when no destPath", function (done) {
    ...
    sourcemaps.write(file, function (err, updatedFile, sourceMapFile) {
      ...
      expect(File.isVinyl(updatedFile)).to.be.true
      expect(updatedFile).toEqual(file)
      expect(updatedFile.contents).toEqual(
        sourceContent + "//# sourceMappingURL=" + base64JSON(updatedFile.sourceMap) + "\n"
      )
      done(err)
    })
  })
  ...
  it("writes an external sourcemap when given a destPath", function (done) {
    ...
    sourcemaps.write(file, "../maps", function (err, updatedFile, sourceMapFile) {
      ...
      expect(File.isVinyl(sourceMapFile)).to.be.true
      ...
      expect(sourceMapFile.stat.isFile()).to.be.true
      expect(sourceMapFile.stat.isDirectory()).to.be.false
      expect(sourceMapFile.stat.isBlockDevice()).to.be.false
      expect(sourceMapFile.stat.isCharacterDevice()).to.be.false
      expect(sourceMapFile.stat.isSymbolicLink()).to.be.false
      expect(sourceMapFile.stat.isFIFO()).to.be.false
      expect(sourceMapFile.stat.isSocket()).to.be.false
  ...
  it("create shortest path to map in file comment", function (done) {
    ...
    sourcemaps.write(file, "dir1/maps", function (err, updatedFile) {
      expect(updatedFile.contents).toEqual(
        sourceContent + "//# sourceMappingURL=../maps/dir1/dir2/helloworld.js.map\n"
      )
      done(err)
    })
  })
