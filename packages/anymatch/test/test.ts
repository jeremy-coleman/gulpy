...
describe("anymatch", () => {
  ...
  describe("with returnIndex = true", () => {
    it("should return the array index of first positive matcher", () => {
      const result = anymatch(matchers, "foo.js", true)
      assert.equal(result, 2)
    })
    it("should return 0 if provided non-array matcher", () => {
      const result = anymatch(matchers[2], "foo.js", true)
      assert.equal(result, 0)
    })
    it("should return -1 if no match", () => {
      const result = anymatch(matchers, "bar.js", true)
  ...
  describe("extra args", () => {
    ...
    it("should pass extra args to function matchers", () => {
      ...
      // curried versions
      const matchFn1 = anymatch(matchers)
      const matchFn2 = anymatch(matchers[4])
