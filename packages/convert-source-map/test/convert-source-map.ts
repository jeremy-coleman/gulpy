...
test("to object returns a copy", t => {
...
test("to map file comment", t => {
  t.equal(
    convert.generateMapFileComment("index.js.map"),
    "//# sourceMappingURL=index.js.map"
  )
  t.equal(
    convert.generateMapFileComment("index.css.map", { multiline: true }),
...
test("from source with a large source", t => {
  const foo = [
    "function foo() {",
    ' console.log("hello I am foo");',
    ' console.log("who are you");',
...
test("remove comments", t => {
  ...
  const // this one is old spec on purpose
    map =
  ...
  t.equal(
    convert.removeComments(foo + map + extraCode),
    foo + extraCode,
    "from the middle of code"
  )
  t.equal(
    convert.removeComments(foo + otherMap + extraCode + map),
    foo + extraCode,
...
test("remove map file comments", t => {
  const foo = [
    "function foo() {",
    ' console.log("hello I am foo");',
    ' console.log("who are you");',
  ...
  t.equal(
    convert.removeMapFileComments(foo + fileMap2),
    foo,
...
test("adding properties", t => {
  const mod = convert.fromJSON(json).addProperty("foo", "bar").toJSON()
...
test("adding properties, existing property", t => {
  try {
    convert.fromJSON(json).addProperty("foo", "bar").addProperty("foo", "bar")
  } catch (error) {
    t.equal(
      error.message,
      'property "foo" already exists on the sourcemap, use set property instead',
...
test("return null fromSource when largeSource is true", t => {
...
test("commentRegex returns new RegExp on each get", t => {
  const foo = [
    "function foo() {",
    ' console.log("hello I am foo");',
    ' console.log("who are you");',
...
test("mapFileCommentRegex returns new RegExp on each get", t => {
  const foo = [
    "function foo() {",
    ' console.log("hello I am foo");',
    ' console.log("who are you");',
