      ...
      try {
        while ((chunk = stream.read()) != null) {
          if (options.outputType)
            if (/^buffer/.test(options.outputType)) assert(Buffer.isBuffer(chunk))
...
function checkEncodeStream(opts) {
  opts.createStream = () =>
    feeder(opts.input).pipe(exports.encodeStream(opts.encoding, opts.encodingOptions))
...
function checkDecodeStream(opts) {
  opts.createStream = () =>
    feeder(opts.input).pipe(exports.decodeStream(opts.encoding, opts.encodingOptions))
    ...
    checkStreamOutput({
      createStream() {
        return feeder(["абв", "где"])
          .pipe(exports.encodeStream("windows-1251"))
          .pipe(exports.decodeStream("windows-1251"))
          .pipe(exports.encodeStream("windows-1251"))
          .pipe(exports.decodeStream("windows-1251"))
      },
      outputType: "string",
...
describe("Streaming sugar", () => {
  it("decodeStream.collect()", done => {
    feeder([
      [0x61, 0x81],
      [0x40, 0x61],
    ])
      .pipe(exports.decodeStream("gbk"))
      .collect((err, outp) => {
        assert.ifError(err)
