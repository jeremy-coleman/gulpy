...
encodingFamilies.forEach(family => {
  family.encodings.forEach(encoding => {
    ...
    if (encoding.aliases)
      encoding.aliases.forEach(alias => {
...
function generateCharsString(encoding) {
  ...
  for (let b = 0x0; b < 0x100; b++) {
    try {
      ...
      if (convertedChar.length != 1)
        throw Error("Single-byte encoding error: Must return single char.")

      const convertedBackBuf = iconvFromUtf8.convert(Buffer.from(convertedChar))
      if (convertedBackBuf.length != 1)
        throw Error("Single-byte encoding error: Cannot decode back.")
  ...
  if (containsDiacritics.length > 0)
    console.log("Contains Diacritics: ", `${containsDiacritics.map(d => d.toString(16))}`)
